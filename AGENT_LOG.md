...
## 2025-09-16T06:45:00Z
- [RB2] Review batch 2 polish: shared utils, CLI hardening, Clap dump CLI, parity enforcement.
- Commands: `pnpm --filter @tf-lang/utils install`, `pnpm install -r`, `pnpm -r --filter "./packages/utils" run build`, `pnpm -r --filter "./packages/tf-lang-l0-ts" run build`, `pnpm -r --filter @tf-lang/adapter-execution-ts run build`, `pnpm -r --filter @tf-lang/trace2tags run build`, `pnpm -r --filter @tf-lang/coverage-generator run build`, `pnpm -r --filter @tf-lang/tf-check run build`, `pnpm -r --filter @tf-lang/* run test`, `cargo test --workspace --all-targets --manifest-path crates/Cargo.toml`, `pnpm --filter @tf-lang/adapter-execution-ts run fixtures` ×2, `pnpm --filter @tf-lang/trace2tags run artifacts` ×2, `pnpm --filter @tf-lang/coverage-generator run artifacts` ×2, `pnpm --filter @tf-lang/tf-check run artifacts` ×2, `pnpm --filter @tf-lang/adapter-execution-ts run parity` ×2.
- Notes: New `@tf-lang/utils` exports underpin canonicalization, repo-root discovery, tmpdir cleanup, and HTML escaping; coverage HTML sanitized & tested; tf-check CLI parser now rejects unknown flags / supports `--k=v`; Rust dump CLI moved to Clap with `--spec/--out`; parity JSON reports `"equal": true` after reruns.
