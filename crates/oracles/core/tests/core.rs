...
#[test]
fn pretty_canonical_json_is_multiline_with_newline() {
...
#[test]
fn deep_equal_reports_nested_object_difference() {
    let left = json!({ "a": { "b": 1, "c": { "x": 1 } } });
    let right = json!({ "a": { "b": 1, "c": { "x": 2 } } });
    let result = deep_equal(&left, &right);
    assert_eq!(result, Err(String::from("/a/c/x")));
}
