...
function canonicalizeInner(value: unknown): unknown {
  ...
  if (value === undefined) {
    throw new TypeError('canonicalize: unsupported value `undefined`');
  }
  ...
  if (typeof value === "bigint") {
    throw new TypeError('canonicalize: unsupported value `bigint`');
  }

  if (typeof value === "symbol") {
    throw new TypeError('canonicalize: unsupported value `symbol`');
  }

  if (typeof value === "function") {
    throw new TypeError('canonicalize: unsupported value `function`');
  }
  ...
  if (value instanceof Date) {
    throw new TypeError('canonicalize: unsupported value `Date`');
  }
...
function canonicalizeSet(set: Set<unknown>): unknown[] {
  const entries = Array.from(set.values()).map((entry) => {
    ...
    const label = canonicalJson(entry);
  ...
  const entries = Object.entries(value)
    .map(([key, entryValue]) => {
      if (entryValue === undefined) {
        throw new TypeError(`canonicalize: unsupported value \`undefined\` at key ${key}`);
      }
