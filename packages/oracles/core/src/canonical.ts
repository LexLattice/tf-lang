...
function canonicalizeInner(value: unknown): unknown {
  ...
  if (value instanceof Date) {
    throw new TypeError("cannot canonicalize Date values");
  }
  ...
  if (typeof jsonLike === "function") {
    return canonicalizeInner(jsonLike.call(value));
  }
  ...
  throw new TypeError(`cannot canonicalize value of unsupported type`);
}
  ...
  const entries = Object.entries(value)
    .map(([key, entryValue]) => {
      if (entryValue === undefined) {
        throw new TypeError(`cannot canonicalize undefined value at key ${key}`);
      }
