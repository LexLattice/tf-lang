...
import { FrameSchema, OrderSchema } from '@tf-lang/pilot-core';
...
function printUsage(message?: string) {
  ...
  console.error(
    'Usage: pilot-strategy run --strategy <momentum|meanReversion> --frames <file> --seed <value> --out <file>'
  );
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const name = options.strategy as Parameters<typeof getStrategy>[0];
