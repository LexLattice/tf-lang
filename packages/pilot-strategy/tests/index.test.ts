...
import { StrategyDefaults, createStrategy, runStrategy } from '../src/index.js';
...
describe('pilot-strategy', () => {
  ...
  it('publishes consistent strategy defaults', () => {
    expect(StrategyDefaults.momentum.breakoutWindow).toBeGreaterThan(0);
    expect(StrategyDefaults.momentum.quantity).toBe('1');
    expect(StrategyDefaults.meanReversion.window).toBeGreaterThan(
      StrategyDefaults.momentum.breakoutWindow,
    );
    expect(StrategyDefaults.meanReversion.delta).toBe('0.15');
  });
});
