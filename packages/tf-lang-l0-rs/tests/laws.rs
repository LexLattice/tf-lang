...
use tflang_l0::model::{Program, Instr};
...
impl Host for DummyHost {
    ...
    fn snapshot_id(&self, snapshot: &serde_json::Value) -> anyhow::Result<String> { Ok(format!("id:{}", serde_json::to_string(snapshot)?)) }
    fn diff_apply(&self, state: &serde_json::Value, delta: &serde_json::Value) -> anyhow::Result<serde_json::Value> {
    ...
    fn journal_record(&self, _plan: &serde_json::Value, delta: &serde_json::Value, s0: &str, s1: &str, _meta: &serde_json::Value) -> anyhow::Result<JournalEntry> {
        Ok(JournalEntry(json!({ "delta": delta, "from": s0, "to": s1 })))
    }
    ...
    fn call_tf(&self, tf_id: &str, args: &[serde_json::Value]) -> anyhow::Result<serde_json::Value> {
        match tf_id {
            "tf://plan/delta@0.1" => {
...
fn vm_basic_ops() -> Result<()> {
    let prog = Program {
        ...
        regs: 6,
        instrs: vec![
            Instr::Const{ dst: 0, value: json!({"state": 1}) },
            Instr::LensProj { dst: 1, state: 0, region: "legal.clauses".into() },
            Instr::LensMerge{ dst: 2, state: 0, region: "legal.clauses".into(), sub: 1 },
            Instr::SnapMake { dst: 3, state: 2 },
            Instr::SnapId { dst: 4, snapshot: 3 },
            Instr::Halt,
        ],
    };
    let vm = VM { host: &DummyHost };
...
fn law_rewind_apply_id_pair_ids_equal() -> Result<()> {
    ...
    let prog = Program {
        ...
        instrs: vec![
            ...
            Instr::Const{ dst: 0, value: json!([1,2]) },
            // r12 = plan value (append 3)
            Instr::Const{ dst: 12, value: json!(3) },
            // r1 = snap0, r2 = id0
            Instr::SnapMake { dst: 1, state: 0 },
            Instr::SnapId   { dst: 2, snapshot: 1 },
            // r3 = delta via CALL (state, plan) -> {"append": plan}
            Instr::Call { dst: 3, tf_id: "tf://plan/delta@0.1".into(), args: vec![0,12] },
            // r4 = state1 after apply
            Instr::DiffApply{ dst: 4, state: 0, delta: 3 },
            // r5 = snap1, r6 = id1
            Instr::SnapMake { dst: 5, state: 4 },
            Instr::SnapId   { dst: 6, snapshot: 5 },
            // r7 = meta {}
            Instr::Const{ dst: 7, value: json!({}) },
            // r8 = journal entry
            Instr::JournalRec { dst: 8, plan: 12, delta: 3, snap0: 2, snap1: 6, meta: 7 },
            // r9 = world_prev = rewind(world_after, entry)
            Instr::JournalRew { dst: 9, world: 4, entry: 8 },
            // r10 = snap_prev, r11 = id_prev
            Instr::SnapMake { dst: 10, state: 9 },
            Instr::SnapId   { dst: 11, snapshot: 10 },
            // r0 = Pack(Pair, id_before, id_prev)
            // r12 = eq(id_before, id_prev)
            Instr::Call { dst: 12, tf_id: "tf://eq/json@0.1".into(), args: vec![2,11] },
            // assert equality
            Instr::Assert { pred: 12, msg: "rewind law violated".into() },
            Instr::Pack { dst: 0, tag: "Pair".into(), regs: vec![2,11] },
            Instr::Halt,
        ],
    };
    let vm = VM { host: &DummyHost };
