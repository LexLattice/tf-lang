...
fn parse_rejects_unknown_op() {
    ...
    assert!(err.to_string().contains("E_SPEC_OP_UNKNOWN at steps[0].op"));
}
...
fn parse_rejects_missing_param() {
    ...
    let err = parse_spec(bad).unwrap_err();
    assert!(err.to_string().contains("E_SPEC_PARAM at steps[0].params.dest"));
}

#[test]
fn parse_rejects_invalid_cpus() {
    let bad = br#"{"version":"0.1","name":"bad","steps":[{"op":"create_vm","params":{"image":"alpine","cpus":0}}]}"#;
    let err = parse_spec(bad).unwrap_err();
    assert!(err.to_string().contains("E_SPEC_PARAM at steps[0].params.cpus"));
}
...
fn parse_rejects_extra_param() {
    ...
    let err = parse_spec(bad).unwrap_err();
    assert!(err.to_string().contains("E_SPEC_PARAM at steps[0].params.extra"));
}
