...
async function main() {
  ...
  try {
    ...
    await runCli(['--ir', inputPath, '--apply', '--out', appliedPath]);
    ...
    for (const law of Array.isArray(postPlan.used_laws) ? postPlan.used_laws : []) {
      if (!known.has(law)) {
        throw new Error(`unknown law reported: ${law}`);
      }
    }
