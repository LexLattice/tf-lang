...
it('imports only sql.js', () => {
  ...
  const better = spawnSync('rg', ['better-sqlite', 'src']);
...
describe('deterministic queries with parameterization', () => {
  ...
  it('reuses prepared statements across filters', () => {
    const a: Filters = { modality: 'FORBIDDEN' };
    const b: Filters = { modality: 'PERMITTED' };
    const r1 = JSON.stringify(count(db, a));
    count(db, b); // mutate bindings
    const r3 = JSON.stringify(count(db, a));
    expect(r3).toBe(r1);
  });
  ...
  it('handles large offsets deterministically', () => {
...
it('400 on invalid filter shapes', async () => {
  const app = buildApp();
  const res = await app.inject({ method: 'GET', url: '/claims/list?limit=0' });
